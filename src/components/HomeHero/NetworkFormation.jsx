...
function targetForFormation(formation, index, total, width, height) {
  ...
  if (formation === 'technology') {
    if (index >= technologyGeometry.nodes.length) return { x: width * 0.72, y: height * 0.49, visible: false }
    const node = technologyGeometry.nodes[index]
    return { x: width * (0.5 + node.nx * 0.42), y: height * (0.13 + node.ny * 0.66), geometryIndex: index, visible: true }
  }
...
export function NetworkFormation({ formation, reducedMotion, onInteractionStart, onInteractionEnd, onFormationSettled }) {
  ...
  useEffect(() => {
    ...
    const applyFormation = () => {
      ...
      nodes.forEach((node, index) => {
        ...
        Object.assign(node, { anchorX: target.x, anchorY: target.y, geometryIndex: target.geometryIndex, major: target.major, visible: target.visible })
      })
    }
    ...
    const render = (time) => {
      ...
      nodes.forEach((node) => {
        ...
        node.vx += (node.anchorX - node.x) * 0.00029 + Math.cos(seconds * 0.2 + node.phase) * 0.0015
        node.vy += (node.anchorY - node.y) * 0.00029 + Math.sin(seconds * 0.28 + node.phase) * 0.0015
        if (pointer.active) {
      ...
      for (let first = 0; first < nodes.length; first += 1) {
        for (let second = first + 1; second < nodes.length; second += 1) {
          ...
          const explicitEdge = isAfrica ? africaEdges.get(edgeKey) : isTechnology ? technologyEdges.get(edgeKey) : isAboutFrame ? aboutFrameGeometry.edges.get(edgeKey) : isEventField ? eventFieldGeometry.edges.get(edgeKey) : null
          const dx = neighbour.x - node.x
          const dy = neighbour.y - node.y
          const distance = Math.hypot(dx, dy)
          if (((isAfrica || isTechnology || isAboutFrame || isEventField) && !explicitEdge) || (!isAfrica && !isTechnology && !isAboutFrame && !isEventField && distance >= CONNECTION_DISTANCE)) continue
          const intensity = (isAfrica ? explicitEdge * 0.58 : isTechnology ? explicitEdge * 0.52 : isAboutFrame ? explicitEdge * 0.42 : isEventField ? explicitEdge * 0.22 : (1 - distance / CONNECTION_DISTANCE) * (isSoundWave ? 0.56 : 0.42)) * Math.min(node.opacity, neighbour.opacity)
      ...
      nodes.forEach((node) => {
        ...
        const opacity = (isSoundWave ? 0.61 + Math.sin(seconds * 1.7 + node.phase) * 0.14 : 0.67 + Math.sin(seconds * 1.7 + node.phase) * 0.12) * node.opacity
        context.fillStyle = `rgba(${isSoundWave ? '234, 247, 242' : '237, 250, 244'}, ${opacity})`
        context.beginPath()
        context.arc(node.x, node.y, node.major ? 1.6 : isSoundWave ? 1.2 : 1.16, 0, Math.PI * 2)
        context.fill()
      })
      ...
      if (!pointer.active && !hasReportedSettlement) {
        ...
        const settledNodes = visibleNodes.filter((node) => (
          Math.hypot(node.anchorX - node.x, node.anchorY - node.y) < SETTLE_DISTANCE
          && Math.hypot(node.vx, node.vy) < SETTLE_SPEED
        ))
        const isVisuallySettled = visibleNodes.length > 0 && settledNodes.length / visibleNodes.length >= 0.95

        if (isVisuallySettled) {
          stableSince ??= time
          if (time - stableSince >= SETTLE_STABILITY_WINDOW) {
  ...
  return (
    <canvas
      ...
      aria-hidden="true"
      onPointerEnter={() => onInteractionStart('network')}
      onPointerLeave={() => onInteractionEnd('network')}
    />
  )
}
